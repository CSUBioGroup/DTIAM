...
def kfold_validation(task, dataset, setting, preset=None, ex_model=[]):
    ...
    if task == 'dti':
        assert dataset in ['yamanishi_08', 'hetionet'], f"dataset should be in ('yamanishi_08', 'hetionet') for {task} task."
        ...
        assert dataset in ['davis','kiba'], f"dataset should be in ('davis','kiba') for {task} task."
        ...
        assert dataset in ['activation', 'inhibition'], f"dataset should be in ('activation', 'inhibition') for {task} task."
        dataset_path = '../data/moa/' + dataset + '/'
        k_folds = 5
        eval_metric = 'roc_auc'
        res_all = pd.DataFrame(columns=['AUROC', 'AUPR'])
    
    assert setting in ['warm_start', 'drug_coldstart', 'protein_coldstart'], "validation setting should be in ('warm_start', 'drug_coldstart', 'protein_coldstart')."
    ...
    for i in range(k_folds):
        ...
        predictor = TabularPredictor(label='y', eval_metric=eval_metric).fit(train_data=train_data, excluded_model_types=ex_model, presets=preset)
        ...
        if task == 'dta':
            ...
            print(f"fold: {i+1}, RMSE: {ret[0]}, MSE: {ret[1]}, Pearson: {ret[2]}, Spearman: {ret[3]}, CI: {ret[4]}")
            res_all.loc[i] = ret
    ...
    res_all.to_csv('../results/' + dataset + '_' + setting + '.csv', index=None, sep='\t')
