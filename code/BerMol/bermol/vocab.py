...
class MolVocab:
    ...
    def build_corpus(self, mol_path, corpus_path=None, update=True):
        ...
        corpus_res = Parallel(n_jobs=-1)(delayed(parall_build)(line.strip(), self.max_len) for line in tqdm(f_mol))
        f_mol.close()
        ...
        with open(corpus_path, 'w') as f_cor:
            for data in tqdm(corpus_res):
                ...
                if len(sentence) > self.max_len:
                    print(sentence)
                f_cor.write('\t'.join([' '.join(sentence), ' '.join(fgs), ' '.join(desc)]) + '\n')
                cnt += 1
        ...
        print("Building Complete, Number of Sample: %s, Vocab Size: %s, Number of Functinal Grounp: %s, Number of Molecular Descriptor: %s" 
    ...
    def save_vocab(self, vocab_path, silent=False):
        with open(vocab_path, "wb") as f:
            pickle.dump(self, f)
        if not silent:
            print(f'Vocab saved. To load, use: vocab = MolVocab.load_vocab("{vocab_path}")')
...
def build():
    ...
    parser.add_argument("--train_mol_path", type=str, required=True, help="input molecular path of training set")
    parser.add_argument("--val_mol_path", type=str, default=None, help="input molecular path of validation set")
    ...
    parser.add_argument("--max_len", type=int, default=128, help="max length of molecular sentence")
    parser.add_argument("--min_freq", type=int, default=1, help="min frequency of the molecular substructure")
    ...
    if config.val_mol_path is not None:
        vocab = vocab.build_corpus(config.val_mol_path, config.val_corpus_path, update=False)
